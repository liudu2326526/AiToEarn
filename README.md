...
- **抖音创作者中心本地自动化**：中国环境可通过本机 `douyin-creator-tools` 和可见 Playwright 浏览器导入抖音评论/已有私信、准备本地发布试填命令，并在试运行后创建人工确认的评论或私信回复任务；需要配置 `DOUYIN_CREATOR_TOOLS_DIR`、`DOUYIN_CREATOR_PROFILE_DIR`，不用于云端无人值守发送。
