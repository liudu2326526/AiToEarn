...
export const ListMonitoredPostQuerySchema = PaginationDtoSchema.extend({
  ...
  monitorStatus: z.enum(['active', 'published', 'paused', 'failed', 'archived']).optional().describe('监控状态'),
  fetchStatus: z.enum(['idle', 'fetching', 'ready', 'failed', 'permission_required', 'not_configured', 'pending_confirmation', 'reviewing']).optional().describe('抓取状态'),
