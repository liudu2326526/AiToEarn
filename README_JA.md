...
- **Douyin Creator Center のローカル自動化**：中国環境では、ローカルの `douyin-creator-tools` と表示状態の Playwright ブラウザを使って Douyin のコメント/既存DM会話を取り込み、ローカル投稿ドライラン用コマンドを準備し、ドライラン確認後にコメントまたはDM返信タスクを作成できます。`DOUYIN_CREATOR_TOOLS_DIR` と `DOUYIN_CREATOR_PROFILE_DIR` の設定が必要で、クラウドでの無人送信は想定していません。
